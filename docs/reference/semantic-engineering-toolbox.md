...
- [Bioregistry](https://bioregistry.io/): A community-driven, centralized registry for identifiers used in the life sciences. The Bioregistry gives an actionable standard for what prefixes should be used in CURIEs, what namespaces should be used in URIs, bridges the gap between CURIEs and URIs, and correctly resolves CURIEs to human-readable webpages (both for OBO terms and many other ontologies/databases). It has an associated Python software package and CLI tool. The Bioregistry incorporates smaller resources like Identifiers.org, Prefix Commons, etc.
